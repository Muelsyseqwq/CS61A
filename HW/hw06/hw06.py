...
def store_digits(n: int):
    ...
    def check(n,lst):
        if n > 9:
            cur,last = n // 10, n % 10
            return check(cur,Link(last,lst))
        else:
            return Link(n,lst)

    return check(n,Link.empty)
...
def deep_map_mut(func, s: Link) -> None:
    ...
    if isinstance(s, Link):
        if isinstance(s.first,int): s.first = func(s.first)
        elif isinstance(s.first,Link): deep_map_mut(func, s.first)
        if s.rest != Link.empty and isinstance(s.rest,Link): deep_map_mut(func, s.rest)
