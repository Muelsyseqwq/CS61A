...
def prune_leaves(t, vals):
    ...
    if is_leaf(t) :
        if label(t) in vals:
            return None # 如果在 就删掉 返回None
        else : return t #否则返回原来的
    else:
        new_branches = [kept for branch in branches(t) for kept in [prune_leaves(branch,vals)] if kept is not None] #全部遍历完分支的新的分支
