...
def num_eights(num):
    ...
    "*** YOUR CODE HERE ***"
    if num == 0: return 0
    return num_eights(num // 10) + int((num % 10) == 8)
...
def interleaved_sum(num, f_odd, f_even):
    ...
    def odds(n):
        if n > num: return 0
        return odds(n + 2) + f_odd(n)

    def evens(n):
        if n > num: return 0
        return evens(n + 2) + f_even(n)
...
def count_dollars(sum_needed):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_function(sum,num):
        if sum == 0: return 1
        elif sum < 0: return 0
        elif num is None: return 0
        return count_function(sum - num,num) + count_function(sum,next_smaller_dollar(num))

    return count_function(sum_needed,100)
...
def count_dollars_upward(sum_needed):
    ...
    def count_function(sum,num):
        ...
        return count_function(sum - num,num) + count_function(sum,next_larger_dollar(num))

    return count_function(sum_needed,1)
...
def move_stack(num, start, end):
    ...
    supp = 6 - start - end
    move_stack(num - 1,start,supp)

    print_move(start,end)

    move_stack(num - 1,supp,end)
