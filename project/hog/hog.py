...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    ones_digit_of_current_player = player_score % 10
    tens_digit_of_opponent_player = (opponent_score // 10) % 10                                                                        
    return max(3 * abs(ones_digit_of_current_player - tens_digit_of_opponent_player),1)
    # END PROBLEM 2
...
def sus_points(score):
    ...
    if num_factors(score) == 3 or num_factors(score) == 4:
        while True:
            if(is_prime(current)):return current
            else:
                current = current + 1
    # END PROBLEM 4
    else :return score
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    score_before_sus = player_score + take_turn(num_rolls, player_score,opponent_score,dice)
    return sus_points(score_before_sus)
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            num_rolls_plyaer0 = strategy0(score0, score1)
            score0 = update(num_rolls_plyaer0,score0,score1,dice)
        else:
            num_rolls_plyaer1 = strategy1(score1, score0)
            score1 = update(num_rolls_plyaer1,score1,score0,dice)
        who = 1 - who
    # END PROBLEM 5
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    strategy1 = strategy(0,0)
    for socre0 in range(goal):
        for socre1 in range(0,goal):
            if socre0 == 0 and socre1 == 0: continue
            strategy2 = strategy(socre0, socre1)
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for i in range(1,11):
        ...
        if current_score > max_score:
            max_score = current_score # 骰子是有状态的  不要调用了两次averaged_dice() 缓存结果
            least_num = i
    return least_num  #结论是骰6个最优 平均值是8.72
    # END PROBLEM 9
...
def final_strategy(score, opponent_score,goal = GOAL):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    boar_score = boar_brawl(score, opponent_score) + score
    points_from_rolling_zero = sus_points(boar_score) - score
    print("aaaa")
    if points_from_rolling_zero  + score >= goal:
        return 0 # 0必胜了
